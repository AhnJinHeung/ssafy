...
public class BOJ_17471_게리맨더링 {
	...
	static void combination(int node, int depth, int cnt) { // depth : 구역 수
		if (cnt == depth) {
			int sum1 = -1, sum2 = -1;
			for (int i=1; i<=N; i++) { // A 선거구
				if (check[i]) {
					sum1 = BFS(i, depth, true);
					break;
				}
			}
			for (int i=1; i<=N; i++) { // B 선거구
				if (!check[i]) {
	...
	static int BFS(int node, int depth, boolean t) { // 선거구 구역 수, sum 계산
